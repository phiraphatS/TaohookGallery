...
<body>
    ...
    <div class="homecontent">
        ...
        <?php endif ?>
        <h1>ดูภาพตัวอย่าง</h1>
        <h3 style="margin-top : 10px;">นี่คือภาพผลงานของ Taohook Gallery ดูเพื่อประกอบการตัดสินใจ</h3>
        ...
        $album_id = $_GET['al_id'];
        $sql = "SELECT * FROM picture_work WHERE album_id = $album_id ";
        $db_query = mysqli_query($conn, $sql);
        $path = "allpicture/";
        while($result = mysqli_fetch_array($db_query)){
        $image = $result['pic_name'];
        
        ?>
            <img src="<?php echo $path.$image; ?>" height="220px" border="1px" alt="" />
        <?php } ?>
